...
show_help() {
    ...
    echo -e "  -cw, --check_cloudwatch_encryption Check if CloudWatch log groups are encrypted with KMS"
    ...
    echo "  -ebs1, --ebs-vol-encryption     Check if EBS volumes are encrypted"
    echo "  -ebs2, --ebs-snap-encryption    Check if EBS snapshots are encrypted"
    ...
    if [ "$choice" == "1" ]; then
        ...
        if [ -z "$IAM_USERS" ]; then
            ...
            for username in $IAM_USERS; do
                #ensure the username is not empty - new 
                if [ -z "$username" ]; then
                    echo "⚠️ Skipping empty username" | tee -a iam_key_rotation.txt
                    continue
                fi
...
# check CloudWatch Log Groups encryption with KMS
check_cloudwatch_encryption() {
    clear
    echo "Do you want to check CloudWatch log encryption for:"
    echo "1. All log groups"
    echo "2. Selected log groups"
    read -p "Please enter your choice (1 or 2): " choice
    identity_info=$(aws sts get-caller-identity --profile "$AWS_PROFILE" --query "{Account:Account, Arn:Arn}" --output json)
    account_id=$(echo $identity_info | jq -r .Account)
    arn=$(echo $identity_info | jq -r .Arn)
    echo_blue "ARN: $arn"
    echo "Checking if CloudWatch log groups are encrypted with KMS..." > cloudwatch_encryption.txt

    if [ "$choice" == "1" ]; then
        aws_command_describe_log_groups="aws logs describe-log-groups --profile "$AWS_PROFILE" --query \"logGroups[].logGroupName\" --output text"
        log_groups=$(eval $aws_command_describe_log_groups)
        echo_yellow "Executed: $aws_command_describe_log_groups" | tee -a cloudwatch_encryption.txt

        if [ -z "$log_groups" ]; then
            echo "❌ No CloudWatch log groups found." | tee -a cloudwatch_encryption.txt
        else
            for log_group in $log_groups; do
                aws_command_get_log_group="aws logs describe-log-groups --profile "$AWS_PROFILE" --log-group-name-prefix \"$log_group\" --query \"logGroups[0].kmsKeyId\" --output text"
                kms_key_id=$(eval $aws_command_get_log_group)
                echo_yellow "Executed: $aws_command_get_log_group" | tee -a cloudwatch_encryption.txt

                if [ "$kms_key_id" != "None" ] && [ -n "$kms_key_id" ]; then
                    echo_green "✅ Log group $log_group is encrypted with KMS key: $kms_key_id" | tee -a cloudwatch_encryption.txt
                else
                    echo_red "❌ Log group $log_group is NOT encrypted with KMS" | tee -a cloudwatch_encryption.txt
                fi
            done
        fi
    elif [ "$choice" == "2" ]; then
        echo "Enter the names of the CloudWatch log groups to check (one per line). Press Ctrl+D when done:"
        mapfile -t selected_log_group_names

        if [ ${#selected_log_group_names[@]} -eq 0 ]; then
            echo "❌ No log group names entered. Exiting..." | tee cloudwatch_encryption.txt
            return 1
        fi

        echo "Checking CloudWatch log encryption for selected log groups..." > cloudwatch_encryption.txt

        for log_group_name in "${selected_log_group_names[@]}"; do
            aws_command_get_log_group="aws logs describe-log-groups --profile "$AWS_PROFILE" --log-group-name-prefix \"$log_group_name\" --query \"logGroups[0].kmsKeyId\" --output text"
            kms_key_id=$(eval $aws_command_get_log_group)
            echo_yellow "Executed: $aws_command_get_log_group" | tee -a cloudwatch_encryption.txt

            if [ "$kms_key_id" != "None" ] && [ -n "$kms_key_id" ]; then
                echo_green "✅ Log group $log_group_name is encrypted with KMS key: $kms_key_id" | tee -a cloudwatch_encryption.txt
            else
                echo_red "❌ Log group $log_group_name is NOT encrypted with KMS" | tee -a cloudwatch_encryption.txt
            fi
        done
    else
        echo "❌ Invalid choice! Exiting script."
    ...
    echo_green "📸 Screenshot taken using Scrot"
    scrot -u -f "IAM_CloudWatchLog_Encryption.png"
}
...
check_ebs_volume_encryption() {
    clear
    echo "Do you want to check EBS Volume encryption for:"
    echo "1. All EBS Volumes"
    echo "2. Selected EBS Volume IDs"
    read -p "Please enter your choice (1 or 2): " choice

    identity_info=$(aws sts get-caller-identity --profile "$AWS_PROFILE" \
    ...
    echo_blue "ARN: $arn"
    echo "Checking EBS Volume Encryption..." | tee ebs_volume_encryption.txt
    if [ "$choice" == "1" ]; then
        aws_cmd_list_vols="aws ec2 describe-volumes --profile \"$AWS_PROFILE\" --query \"Volumes[].VolumeId\" --output text"
        volumes=$(eval "$aws_cmd_list_vols")
        echo_yellow "Executed: $aws_cmd_list_vols" | tee -a ebs_volume_encryption.txt
        if [ -z "$volumes" ]; then
            echo_red "❌ No EBS volumes found." | tee -a ebs_volume_encryption.txt
            return 1
        fi
    elif [ "$choice" == "2" ]; then
        echo "Enter EBS Volume IDs (one per line). Press Ctrl+D when done:"
        mapfile -t volume_list

        if [ ${#volume_list[@]} -eq 0 ]; then
            echo_red "❌ No Volume IDs entered. Exiting..." | tee -a ebs_volume_encryption.txt
            return 1
        fi
        volumes="${volume_list[@]}"
    else
    ...
    for vol in $volumes; do
        aws_cmd_vol_desc="aws ec2 describe-volumes --profile \"$AWS_PROFILE\" --volume-ids \"$vol\" \
            --query \"Volumes[0].{Encrypted:Encrypted, KmsKeyId:KmsKeyId, Size:Size}\" --output json"
        echo_yellow "Executed: $aws_cmd_vol_desc" | tee -a ebs_volume_encryption.txt
        vol_info=$(eval "$aws_cmd_vol_desc")
        encrypted=$(echo "$vol_info" | jq -r .Encrypted)
        kmskey=$(echo "$vol_info" | jq -r .KmsKeyId)
        size=$(echo "$vol_info" | jq -r .Size)
        vol_arn="arn:aws:ec2:$region:$account_id:volume/$vol"
        echo "Volume ID: $vol (ARN: $vol_arn)" | tee -a ebs_volume_encryption.txt
        if [ "$encrypted" == "true" ]; then
            echo_green "✅ Encrypted (KMSKey: $kmskey) | Size: ${size}GB" | tee -a ebs_volume_encryption.txt
        else
            echo_red "❌ NOT Encrypted | Size: ${size}GB" | tee -a ebs_volume_encryption.txt
        fi
        echo "---------------------------------" | tee -a ebs_volume_encryption.txt
    done

    echo_green "📸 Screenshot taken using Scrot"
    scrot -u -f "EBS_Volume_Encryption.png"
}

# check if EBS Snapshots are encrypted
check_ebs_snapshot_encryption() {
    clear
    echo "Do you want to check EBS Snapshot encryption for:"
    echo "1. All snapshots owned by this account"
    echo "2. Selected Snapshot IDs"
    read -p "Please enter your choice (1 or 2): " choice
    ...
    echo_blue "ARN: $arn"
    echo "Checking EBS Snapshot Encryption..." | tee ebs_snapshot_encryption.txt

    if [ "$choice" == "1" ]; then
        aws_cmd_snap="aws ec2 describe-snapshots --owner-ids self --profile \"$AWS_PROFILE\" \
            --query \"Snapshots[].SnapshotId\" --output text"
        snapshots=$(eval "$aws_cmd_snap")
        echo_yellow "Executed: $aws_cmd_snap" | tee -a ebs_snapshot_encryption.txt
        if [ -z "$snapshots" ]; then
            echo_red "❌ No snapshots found." | tee -a ebs_snapshot_encryption.txt
            return 1
        fi
    elif [ "$choice" == "2" ]; then
        echo "Enter Snapshot IDs (one per line). Press Ctrl+D when done:"
        mapfile -t snap_list
        if [ ${#snap_list[@]} -eq 0 ]; then
            echo_red "❌ No Snapshot IDs entered. Exiting..." | tee -a ebs_snapshot_encryption.txt
            return 1
        fi
        snapshots="${snap_list[@]}"
    else
    ...
    for snap in $snapshots; do
        aws_cmd_snap_desc="aws ec2 describe-snapshots --profile \"$AWS_PROFILE\" \
            --snapshot-ids \"$snap\" \
            --query \"Snapshots[0].{Encrypted:Encrypted, KmsKeyId:KmsKeyId, VolumeSize:VolumeSize}\" --output json"

        echo_yellow "Executed: $aws_cmd_snap_desc" | tee -a ebs_snapshot_encryption.txt
        snap_info=$(eval "$aws_cmd_snap_desc")
        encrypted=$(echo "$snap_info" | jq -r .Encrypted)
        kmskey=$(echo "$snap_info" | jq -r .KmsKeyId)
        size=$(echo "$snap_info" | jq -r .VolumeSize)
        snap_arn="arn:aws:ec2:$region:$account_id:snapshot/$snap"
        echo "Snapshot ID: $snap (ARN: $snap_arn)" | tee -a ebs_snapshot_encryption.txt
        if [ "$encrypted" == "true" ]; then
            echo_green "✅ Encrypted (KMSKey: $kmskey) | Size: ${size}GB" | tee -a ebs_snapshot_encryption.txt
        else
            echo_red "❌ NOT Encrypted | Size: ${size}GB" | tee -a ebs_snapshot_encryption.txt
        fi
        echo "---------------------------------" | tee -a ebs_snapshot_encryption.txt
    done
    echo_green "📸 Screenshot taken using Scrot"
    scrot -u -f "EBS_Snapshot_Encryption.png"
}
    ...
    while [[ $# -gt 0 ]]; do
	    case "$1" in
			...
			-sm|--secrets)
			    check_secrets
				;;
            -ebs1|--ebs-vol-encryption)
                check_ebs_volume_encryption
                shift
                ;;
            -ebs2|--ebs-snap-encryption)
                check_ebs_snapshot_encryption
                shift
                ;;
		    -cl|--consoleLink)
